...
def evaluate_case(i):
    ...
    gt_centers = load_json(f'homework_1/train/{TRAIN_CASES[i]}_centers.json')
    gt_n_trains = load_json(f'homework_1/train/{TRAIN_CASES[i]}_n_trains.json')
    gt_scores = load_json(f'homework_1/train/{TRAIN_CASES[i]}_scores.json')

    img = cv2.imread(f'homework_1/train/{TRAIN_CASES[i]}.jpg')
